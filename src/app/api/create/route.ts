...
export async function POST(request: NextRequest) {
  ...
  const { topic, difficulty, size, isSubscribed } = await request.json();

  const q = query(collection(db, "flashcards"), where("userId", "==", userId));
  const querySnapshot = await getDocs(q);

  if (!isSubscribed && querySnapshot.docs.length >= 5) {
    return NextResponse.json(
      {
        error:
          "You have reached the maximum limit of flashcard sets on a free plan.",
      },
      { status: 403 }
    );
  }
